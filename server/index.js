...
app.get('/callback', (req, res) => {
   ...
   function setUserSessionGetAuthAccessToken (userInfoResponse) {
  ...
  .then(userInfoResponse => setUserSessionGetAuthAccessToken(userInfoResponse))
...
app.get('/api/star', (req, res) => {
  ...
  axios.put(`https://api.github.com/user/starred/${gitUser}/${gitRepo}?access_token=${req.session.access_token}` 
  ).then(response => {
    res.status(200).end()
  }).catch(error => console.log(error))
})
...
app.get('/api/unstar', (req, res) => {
  const { gitUser, gitRepo } = req.query;
  axios.delete(`https://api.github.com/user/starred/${gitUser}/${gitRepo}?access_token=${req.session.access_token}`
).then(response => {
  res.status(200).end()
}).catch(error => console.log(error))
})
